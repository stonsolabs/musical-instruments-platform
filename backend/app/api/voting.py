...
async def get_most_voted_products(
    ...
    vote_stats_subq = (
        select(
            ...
            func.sum(func.case((ProductVote.vote_type == 'up', 1), else_=0)).label('thumbs_up_count'),
            func.sum(func.case((ProductVote.vote_type == 'down', 1), else_=0)).label('thumbs_down_count'),
            func.count().label('total_votes'),
            (func.sum(func.case((ProductVote.vote_type == 'up', 1), else_=0)) - 
             func.sum(func.case((ProductVote.vote_type == 'down', 1), else_=0))).label('vote_score')
        )
