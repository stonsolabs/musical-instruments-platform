...
async function searchProducts(params: any): Promise<{ products: Product[] }> {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/products?${sp.toString()}`);
...
export default function HomePage({ initialPopularProducts, initialTopRatedProducts }: HomePageProps) {
  ...
  // Load fresh data on client side for real-time updates
  useEffect(() => {
  ...
  return (
    <>
      <Head>
        <title>Compare Musical Instruments - Find the Best Deals</title>
        <meta name="description" content="Find the best deals on musical instruments. Compare specs, read reviews, and discover your next instrument from Europe's top music stores." />
        <meta name="keywords" content="musical instruments, guitar, piano, keyboard, drums, compare prices, music gear" />
        
        {/* Open Graph */}
        <meta property="og:title" content="Compare Musical Instruments - Find the Best Deals" />
        <meta property="og:description" content="Find the best deals on musical instruments. Compare specs, read reviews, and discover your next instrument." />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://your-domain.com" />
        
        {/* Twitter Card */}
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Compare Musical Instruments - Find the Best Deals" />
        <meta name="twitter:description" content="Find the best deals on musical instruments. Compare specs, read reviews, and discover your next instrument." />
...
export const getServerSideProps: GetServerSideProps<HomePageProps> = async (context) => {
  try {
    // Fetch initial data for SSR - this ensures the page loads with content
    // and improves SEO and Core Web Vitals
    const [popularResponse, topRatedResponse] = await Promise.all([
      fetch(`${API_BASE_URL}/products?page=1&limit=6&sort_by=popularity`),
      fetch(`${API_BASE_URL}/products?page=1&limit=6&sort_by=rating`)
    ]);

    const initialPopularProducts = popularResponse.ok ? await popularResponse.json() : { products: [] };
    const initialTopRatedProducts = topRatedResponse.ok ? await topRatedResponse.json() : { products: [] };

    return {
      props: {
        initialPopularProducts: initialPopularProducts.products || [],
        initialTopRatedProducts: initialTopRatedProducts.products || [],
      },
